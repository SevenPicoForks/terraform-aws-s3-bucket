...
variable "create_bucket_policy" {
  ...
  default     = true
  description = <<-EOT
    Set to `false` to prevent the underlying s3-bucket module from creating an
    `aws_s3_bucket_policy` resource. Passed through directly to the root module's
    `create_bucket_policy` variable. Use when an external Terraform stage (e.g.,
    `ou-management/01-iam`) is the designated owner of this bucket's policy.
    EOT
}
#
